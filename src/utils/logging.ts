...
export function logPythonException(exceptionContent: PythonExceptionMessage) {
    ...
    output.append(
        `Python error '${exceptionContent.exception}' occured: ${exceptionContent.message}\n`
    );
    vscode.window
        .showErrorMessage(
            `NiSight: Python error ${exceptionContent.exception} occured. Show the output console for details...`,
